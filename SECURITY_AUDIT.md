...
### SEC-059 — Zoning & Land Check: new surfaces (endpoints, upload, proxies, CDN, CSP) — built to baseline
- **Status:** fixed
- **Severity:** low
- **Category:** New Feature Attack Surface (review)   **OWASP:** A01/A03/A05/A10   **Confidence:** High
- **Affected:** `api/zoning_api.php`, `admin/zoning.php`, `zoning.js`, `index.html`, `.htaccess`, `tools/zoning_ingest.mjs`
- **Description:** ADR 0013 adds a public API (`zoning_api.php`), a hardened certificate upload, two outbound proxies (Photon geocode, BHUMI WMS GetFeatureInfo), a standalone admin page, a CDN dependency (Leaflet) and a CSP change. Controls applied inline: `_sec.php` bootstrap (session flags, same-origin, generic errors); `sec_require_csrf` on every write + frontend `X-CSRF-Token`; all SQL via PDO prepared (point-in-polygon binds a float-cast WKT); JSON-only output (frontend escapes via `zEsc`); both proxies use `safe_fetch` (SSRF-guarded) with admin-configured base URLs and coordinates constrained to the NTB window (`valid_latlng`); per-IP rate limits on check/geocode/profile/request/upload; report access gated by owner uid OR unguessable `access_token` (hash_equals) OR admin; uploads = finfo MIME whitelist (pdf/jpg/png), 8MB cap, randomised name, stored **outside web root** (`/home/rovin629/zoning_uploads`, chmod 600), never web-served, ownership/token checked; Leaflet pinned with SRI on the already-allowed cdnjs origin; CSP `style-src` extended to cdnjs for Leaflet CSS only. Admin page: `sec_session_start('Strict')`, admin auth, login rate-limit, CSRF-validated forms, `htmlspecialchars` output, status whitelist.
